		...
		try {
			...
			while ((info = enumerator.next_file()) != null) {
				...
				if(!uint64.try_parse(name, out pid)) {
					// make sure to not access any other directorys that may be present in /proc for some reason
					continue;
				}
				...
				string[] splitted = parseline.split(" ", 4); // we are not interested in the part after ppid so just leave it a big string
				...
				uint64 ppid = uint64.parse(splitted[2]);
